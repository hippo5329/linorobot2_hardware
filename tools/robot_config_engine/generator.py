...
def generate_config_header(spec: Dict[str, Any]) -> str:
    ...
    if bat_type == "ADC_DIVIDER":
        ...
        lines.append(f"#define BATTERY_R1 {r1:.1f} // Upper resistor (Ohms)")
        lines.append(f"#define BATTERY_R2 {r2:.1f} // Lower resistor (Ohms)")
        lines.append("// 1000pF filter capacitor recommended across ADC pin to GND for noise reduction")
        lines.append("// Ensure ADC input voltage never exceeds 3.0V maximum")
        if sensors.get("adc_lut"):
            lut = sensors["adc_lut"]
            lines.append("#define USE_ADC_LUT")
            if isinstance(lut, list):
                lut_str = ", ".join(str(x) for x in lut)
                lines.append(f"const int16_t ADC_LUT[4096] = {{ {lut_str} }};")
            else:
                lines.append(f"const int16_t ADC_LUT[4096] = {{ {lut} }};")
            lines.append(f"#define BATTERY_ADJUST(v) (ADC_LUT[v] * (3.3 / 4096.0 * ({r1:.1f} + {r2:.1f}) / {r2:.1f} * 1.0))")
        else:
            lines.append(f"#define BATTERY_ADJUST(v) ((v) * (({r1:.1f} + {r2:.1f}) / {r2:.1f}) / 1000.0)")
    ...
    if sensors.get("sonar", False):
        ...
        lines.append(f"#define TRIG_PIN {sonar.get('trig', 5)}")
        lines.append(f"#define ECHO_PIN {sonar.get('echo', 15)}")

    lines.append("")
    lines.append("// Default Baudrate (921600 Baud standard)")
    lines.append(f"#define BAUDRATE {spec.get('baudrate', 921600)}")
    ...
    enable_wifi_net = is_wifi_transport or bool(spec.get("enable_ota_syslog", False))

    if is_wifi_transport:
        agent_ip_str = wifi.get("agent_ip", "192.168.1.100") if wifi else "192.168.1.100"
        ip_parts = [p.strip() for p in agent_ip_str.split(".")]
        ip_formatted = f"{{ {', '.join(ip_parts)} }}" if len(ip_parts) == 4 else "{ 192, 168, 1, 100 }"
        ssid = wifi.get("ssid", "YOUR_WIFI_SSID") if wifi else "YOUR_WIFI_SSID"
        password = wifi.get("password", "YOUR_WIFI_PASSWORD") if wifi else "YOUR_WIFI_PASSWORD"
        agent_port = wifi.get("agent_port", 8888) if wifi else 8888
    ...
    # Dual-Core Task Allocation (FreeRTOS)
    dual_core = spec.get("dual_core", True)
    if dual_core:
        lines.append("\n#define USE_DUAL_CORE // FreeRTOS dual-core task allocation (Strict 50Hz PID on Core 1)")
    else:
        lines.append("\n// #define USE_DUAL_CORE // Dual-core task allocation disabled (Single-core mode)")

    # Hardware Task Watchdog (WDT)
    wdt = spec.get("watchdog", {})
    if isinstance(wdt, dict) and wdt.get("enabled", False):
        timeout = wdt.get("timeout_sec", 60)
        lines.append(f"\n#define WDT_TIMEOUT {timeout} // Hardware Task Watchdog Timeout (Seconds)")
    elif isinstance(wdt, (int, float)) and wdt > 0:
        lines.append(f"\n#define WDT_TIMEOUT {int(wdt)} // Hardware Task Watchdog Timeout (Seconds)")
    else:
        lines.append("\n// #define WDT_TIMEOUT 60 // Hardware Task Watchdog disabled")
...
{wifi_flag}"""
        ...
        serial_iface = spec.get("serial_interface", "CDC").upper()
        is_bridge = serial_iface == "BRIDGE"
